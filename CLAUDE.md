...
The project is a Maven multi-module build: an aggregator/parent `pom.xml` with four modules — `common` (`scheduler-common`, plain-Java shared business rules with no framework/persistence dependencies — `RoomTypeCompatibility` and `CalendarPacking`, see below), `engine` (`scheduler-engine`, Timefold + JDBC, no Spring), `reporter` (`scheduler-reporter`, reads the solved schedule from the database and generates PDF reports; depends on `scheduler-engine` as a library), and `web` (`scheduler-web`, Spring Boot REST API). `engine` and `web` both depend on `common` for shared rules, but do not depend on each other; all three modules that touch the database integrate only through the shared PostgreSQL schema. `common` exists specifically so a rule shared by the solver and the web API has exactly one implementation instead of two hand-synchronized copies. The React frontend lives in `web-ui/` (unchanged).

**`common.CalendarPacking`** (added 2026-09-05): the canonical day-by-day bin-packing algorithm (pack hours into blocks, check a block count against a day cap, greedily assign block lengths into a day-keyed calendar of contiguous hour windows) — extracted after `engine`'s `PreSolveValidator.validateSharedTeacherLoad` (see below) needed the exact same reasoning `web`'s `AvailabilityAwareBlockShaper` already had, and hand-copying it a second time (as `PreSolveValidator` initially did, `Integer` day-keys re-typed to `java.time.DayOfWeek`) would have created the same "two implementations to keep in sync" problem `common` exists to prevent. The day-key type is a Java generic (`<D>`) wherever the algorithm needs to track per-day state (`assignWindows`) — it doesn't care whether the caller's calendar is keyed by `Integer`, `DayOfWeek`, or anything else with working `equals`/`hashCode` — and a plain wildcard (`Map<?, List<int[]>>`) everywhere the algorithm only reads a windows map's values. Building the initial calendar from a teacher's raw availability (and any pinned-hours subtraction) stays genuinely module-specific glue, not moved here — only the packing/placement algorithm itself is shared. `AvailabilityAwareBlockShaper` (web) is now a thin `TeacherEntity`-facing facade over this class; `PreSolveValidator`'s `windowsMinusPinned`/`greedyAssign` delegate to it directly. Has its own direct test class (`CalendarPackingTest`, in `common`), including cases using a `String`-keyed calendar specifically to prove the generic methods don't secretly depend on either module's own concrete day-key type.
...
8. `semesterHourLimitsMustBeRespected` - A block whose course has a HARD-severity `semester_hour_limit` configured (`course.getLatestEndHour()`/`getLatestEndHourSeverity()`, edited per course via Settings > Semester Hour Limits) may never be assigned a timeslot ending after that course's `latestEndHour`. NOT excluded for pinned assignments (a data-integrity check, like `blockLengthMustMatchTimeslotLength`/`teacherRequiredRoomMustBeUsed`): a non-pinned block's timeslot is already structurally guaranteed correct by `CourseBlockAssignment.getMatchingBlockTimeslots()`'s entity-scoped value range (it excludes any timeslot ending past a HARD-severity course's limit from the range entirely), so in practice this only ever fires for a pinned row whose timeslot predates the limit (or was pinned before its course was configured with one). Reused directly by `PreSolveValidator` (`BlockScheduleMath.violatesHardSemesterHourLimit()`) so the two can't drift apart. Originally added 2026-08-27 as a hardcoded "semester 1 must finish by 14:00" rule, then generalized 2026-09-01 into this per-course, HARD-or-SOFT-configurable limit (commit `0d1135c`) - see `preferSemesterHourLimits` below for the SOFT-severity counterpart. Mirrored in `BlockScheduleAnalyzer`.
...
4. `preferSemesterHourLimits` (weight 6) - The SOFT-severity counterpart to the HARD `semesterHourLimitsMustBeRespected` constraint above: for a course whose `semester_hour_limit` is configured as SOFT rather than HARD, prefer its blocks finish by `latestEndHour`, but the solver may still place them later - penalized proportionally to how far past the limit they end (`BlockScheduleMath.softSemesterHourLimitExcess()`, a deviation gradient like `preferSemesterOneBlocksStartEarly`'s, not a flat penalty). Pinned assignments excluded. Added 2026-09-01 alongside the HARD variant's generalization (commit `0d1135c`).
5. `teacherMaxHoursPerWeek` (weight 5) - Minimize teacher workload violations
6. `roomCapacityShouldFitGroupSize` (weight 4) - Group's `studentCount` shouldn't exceed the assigned room's `capacity`; only checked when both values are known.
7. ~~`minimizeGroupIdleGaps` (weight 3)~~ **TEMP DISABLED 2026-08-24** (per request - replaced for first-semester groups by `minimizeSemesterOneGroupIdleGaps` above; other groups' idle gaps are no longer minimized at all). Minimizes idle time between blocks for student groups. Penalizes only ADJACENT block pairs (via `forEachUniquePair` + `ifNotExists`) so idle hours are counted once per gap, not re-counted by non-adjacent pairs. `BlockScheduleAnalyzer` mirrors this by grouping per (group, day), sorting, and summing adjacent-block gaps.
8. `preferBlockSpecifiedRoom` (weight 3) - Prefer room specified in `assignment.getPreferredRoomHint()` field
9. `groupPreferredRoomConstraint` (weight 2) - Groups prefer a room from their curated `group_room_range` for the block's `satisfiesRoomType` (see "Dynamic Room Assignment" below); keyed by room type, so it naturally applies only when the group has actually curated a range for that specific type
10. `minimizeTeacherIdleGaps` (weight 2) - Reduce gaps between blocks for same teacher on same day. Availability-aware (only counts idle hours the teacher is actually available) and penalizes only ADJACENT block pairs (via `forEachUniquePair` + `ifNotExists`) to avoid re-counting the same idle hours across non-adjacent pairs.
11. ~~`minimizeTeacherBuildingChanges` (weight 1)~~ **TEMP DISABLED 2026-08-24** (per request - not required anymore) - Reduce building switches for teachers on same day
12. ~~`preferCoreOneHourBlocksAtSameTimeAcrossDays` (weight 2)~~ **TEMP DISABLED 2026-08-26** (per request) - For a `Core`-designation course's 1-hour blocks (one per day it meets, same group), prefer they all land on the same start hour - a predictable "Math is always at 8am" schedule. Doesn't apply to multi-hour blocks or pinned assignments. Penalty is the deviation from the most common ("mode") start hour among the group's blocks for that course, not a flat all-or-nothing penalty. Mirrored in `BlockScheduleAnalyzer`.
...
`PreSolveValidator.validate(schedule)` runs immediately after `DataLoader`, before the solver is even built (`MainBlockSchedulingApp`) — both the CLI and the web "Run Solver" button (`EngineController` → `EngineRunnerService`, which shells out to `scripts/run-engine.sh`) funnel through this same entry point; a "Run Validation" Tools page (`/validation`, `PreSolveValidationController`/`PreSolveValidationRunnerService`, open to WRITER and ADMIN) runs the identical check via `VALIDATE_ONLY=true` without ever reaching the solve phase, for a fast up-front report on its own. Ten of the eleven checks it runs are blocking `problems`: a non-empty result prints the full report and calls `System.exit(1)` without solving at all (or, under `VALIDATE_ONLY`, just exits with a non-zero code). The eleventh (`validateSharedTeacherLoad`, added 2026-09-05 — see below) is deliberately a non-blocking `warning` instead, printed either way but never aborting the run. Eleven checks total, in three groups:
...
These ten are deliberately blocking rather than advisory: each is a proven mathematical/structural fact, not a judgment call, so letting a solve run anyway only ever burns the full time budget to confirm what's already provable. `SKIP_PRESOLVE_VALIDATION=true` (CLI env var; `EngineRunRequest.skipValidation` / a checkbox on Settings' Solver tab for the web trigger) is a blanket escape hatch for testing/debugging — validation still runs and prints everything it finds either way, the flag only changes whether a non-empty result aborts the run. Deliberately all-or-nothing rather than a `constraint_config`-style per-check DB toggle: since every one of these ten is a proven fact rather than a judgment call, there's no legitimate reason to disable one selectively while leaving the others on.

**Advisory (warning, never blocking)**:
- `validateSharedTeacherLoad` (added 2026-09-05) — the validation-side mirror of the exact blind spot `BlockGenerationService`'s shape adaptation used to have (see "Shared per-teacher calendar" below): `validateBlockSpreadCapacity` above checks each (group, course) pair's day requirement against a teacher's FULL raw day count in isolation, never accounting for what the teacher's OTHER pairings also need. For a teacher shared across 2+ (group, course) pairings, this greedily simulates (largest-remaining-movable-hours-first) whether all their MOVABLE blocks can be packed into the teacher's actual per-day hour windows — after exactly subtracting hours already committed to that teacher's PINNED assignments — without any pairing exceeding its own `maxBlocksPerDay` cap. Confirmed live (2026-09-05): a teacher shared across 9 groups, individually fine and passing every check above, still left the solver with 2 double-bookings after both a normal and a 30-minute solve with a fresh random seed. Deliberately a **warning**, unlike every check above: this is a greedy heuristic, not a proof — a failed simulation means "this placement order didn't work," not "no arrangement can ever work," so flagging it as blocking would risk aborting solves that are actually perfectly feasible via a smarter order than this simulation tried. Populates `ValidationResult.getWarnings()`, exactly the case that field's own javadoc was written to anticipate.
...
**Availability-aware block generation** (`BlockGenerationService`'s no-template fallback, `AvailabilityAwareBlockShaper`; full mechanics, stage-by-stage, in `docs/generate-blocks.md`): a component's default block decomposition (its `component_block_rule.preferredBlockSize`, or `DEFAULT_BLOCK_SIZE`) is teacher-blind by default, which is exactly why `PreSolveValidator.validateBlockSpreadCapacity` (see below) can find a course needing more distinct days than its resolved teacher actually has. When a teacher is already resolved (`group_course.default_teacher_id`) and the naive shape wouldn't leave at least `AvailabilityAwareBlockShaper.DEFAULT_MARGIN_DAYS` (1) spare distinct day beyond the minimum needed under `component_block_rule.maxBlocksPerDay` — not just bare feasibility — `decomposeHours` asks `AvailabilityAwareBlockShaper.tryAvailabilityAwareShape()` for the first longer block size (up to the 4h structural max) that does have that margin; fewer, longer blocks need fewer days at the same daily cap. The margin requirement exists because a shape that's only just barely possible leaves the solver zero room to absorb any other scheduling pressure that day — confirmed live (2026-09-05): two pairs generated with exactly zero slack both went on to violate `maxBlocksPerDay` once solved. This is a probabilistic hedge, not a guarantee (a third pair that same night had a full spare day and still got violated). When no size reaches the margin, `tryAvailabilityAwareShape` gracefully settles for a merely bare-feasible shape rather than giving up outright — a course already needing every one of the teacher's available days has nowhere left to find a spare day from. This never assigns a specific day; the solver still freely places each block among the teacher's available days, exactly as for any other generated block. Falls back to the naive shape unchanged when there's no teacher yet, no availability data to reason from, or nothing up to 4h is even bare-feasible — a genuinely infeasible pairing `PreSolveValidator` still reports, exactly as before this existed.

**Shared per-teacher calendar across pending pairings ("Option C", added 2026-09-05)**: several (group, course) pairings sharing one teacher in a single `generateBlocks()` run reason against one shared, progressively-consumed calendar instead of each assuming they have the teacher's whole week to themselves — processed largest-hours-first, tied-broken by ascending course semester then group id (so the outcome is reproducible, not an accident of database iteration order). A dataset-wide simulation found 16 pairs came out shaped differently once this was fixed, not an isolated coincidence. See `docs/generate-blocks.md`'s "Stage 0.5" section for the full mechanics.

**Effective calendar: a teacher's own existing load, not just sibling pairings (added 2026-09-05)**: extends Option C to a teacher's *pre-existing* assignments too, not just pairings pending in the same run — a PINNED existing assignment's hours are subtracted from the calendar exactly; a still-movable one can't be (no placed day yet), so its presence instead adds one required margin day. See `docs/generate-blocks.md`'s "Effective calendar" subsection for the full mechanics.

**Prefer minimal block-size upgrades for Core courses (added 2026-09-05)**: per request, `Core` specifically (hardcoded by name, not driven by whatever `preferredBlockSize` happens to be configured for any component in general) keeps as many preferred-size blocks as possible when margin adaptation is needed, merging only as many pairs of blocks into double-size ones as actually needed (e.g. `[2, 1, 1]` rather than `[2, 2]`) — doubling, not a flat "+1", being the only upgrade that exactly preserves total hours — and never escalates beyond that doubled size even when it's not enough. See `docs/generate-blocks.md`'s "Core's minimal-upgrade preference" subsection for the full mechanics.

**Configurable margin per component (added 2026-09-05)**: `component_block_rule.marginDays` (optional) lets a scheduler override the hardcoded `AvailabilityAwareBlockShaper.DEFAULT_MARGIN_DAYS` (1) per component instead of one value for the whole school, surfaced in Settings' Block Rules tab. See `docs/generate-blocks.md`'s "Configurable margin" subsection for the full mechanics.

**Surfacing what got reshaped (added 2026-09-05)**: `generateBlocks()`'s response gains an `adjustments` list (`GenerationResult.getAdjustments()`), separate from `warnings` (which is for actual problems) — one entry per portion whose chosen shape differed from its naive one, so a scheduler can see what got reshaped without reconstructing it from the database by hand. See `docs/generate-blocks.md`'s Output section for the full mechanics.
