...
**Shared per-teacher calendar across pending pairings ("Option C", added 2026-09-05)**: `decomposeHours` above reasons about a teacher's *remaining* availability, not always their full raw availability — because a dataset-wide simulation found that reasoning about each (group, course) pairing in isolation, when the *same* teacher is `default_teacher_id` for several pairings needing generation in one `generateBlocks()` run (e.g. one teacher teaching 5 different groups the same course), let every pairing believe it had the teacher's whole calendar to itself; 16 pairs across the real dataset came out shaped differently once this was fixed, not an isolated coincidence. `generateBlocks()` now groups all pending pairings by resolved teacher first: a teacher with only one pending pairing is unaffected (`sharedCalendar = null`, exactly today's behavior, verified byte-for-byte unchanged against every pre-existing test); a teacher with 2+ builds one shared calendar (see "Effective calendar" below), processes those pairings **largest-hours-first, tied-broken by ascending course semester, then by group id** (added 2026-09-05: a lower-semester group gets first claim on the calendar's more comfortable shape when hours tie, consistent with how this system already privileges semester-1 groups elsewhere, and the whole ordering is now reproducible across reruns of identical data instead of an accident of database iteration order), and after each pairing's shape is chosen, consumes its hours from the shared map (`consumeFromCalendar` → `assignWindows`) before the next pairing in line is decomposed — so a later pairing sees a genuinely reduced calendar, not the full one, while a non-exclusive teacher's blocks are still left dayless for the solver to place (only the *hours consumed* matter to the next pairing, not a committed day). This is why `AvailabilityAwareBlockShaper`'s core methods (`windowsByDay`, `distinctAvailableDayCount`, `largestContiguousWindow`, `tryAvailabilityAwareShape`, `assignWindows`) each now have a `Map<Integer, List<int[]>>`-based form alongside the original `TeacherEntity`-based one, and why the `Map`-based `assignWindows` is transactional (deep-copies before mutating, commits only on full success) — the map can now be shared and reused across several pairings' calls, unlike its old single-use-per-call shape, so a failed attempt must leave it untouched for whichever pairing tries next. Stage 4 exclusive-teacher pinning is untouched by this — its own trigger condition (teacher has *no other* pairing at all) is mutually exclusive with belonging to a 2+-pairing shared group.

**Effective calendar: a teacher's own existing load, not just sibling pairings (added 2026-09-05)**: Option C's shared calendar above only protected pairings pending *in the same run*. A brand-new pairing for a teacher who already has other, previously-generated (or manually created) assignments was still shaped against their full raw availability, blind to hours those existing assignments already claim — the same blind spot Option C fixed, recurring one level up across runs instead of within one. `BlockGenerationService.buildEffectiveCalendar(teacher, existingAssignments)` closes this: it now runs for *every* resolved teacher, not just when 2+ pairings are pending together. A **pinned** existing assignment has a known day/hour, so its hours are subtracted from the calendar exactly (`AvailabilityAwareBlockShaper.windowsByDay(teacher, consumedRanges)`, a new overload that carves specific hours out of a teacher's raw availability before computing windows — removing an hour from the middle of a run splits it into two windows automatically). A still-**movable** (non-pinned) existing assignment can't be subtracted the same way (no placed day yet), so its mere presence instead adds one extra required margin day (`EXTRA_MARGIN_DAYS_FOR_EXISTING_MOVABLE_LOAD`) — coarser than exact subtraction, but still meaningfully conservative rather than pretending that load doesn't exist at all. A teacher with no existing assignments (the common case, and always true for an exclusive teacher) gets back exactly the unmodified raw calendar.

**Prefer minimal block-size upgrades for Core courses (added 2026-09-05)**: per request, the `Core` designation specifically (hardcoded by name, not driven by whatever `preferredBlockSize` happens to be configured for any component in general) favors keeping as many preferred-size blocks as possible when margin adaptation is needed, rather than uniformly resizing every block the way `tryAvailabilityAwareShape` does for every other designation. `AvailabilityAwareBlockShaper.tryMinimalUpgradeShape()` finds the largest block count (fewest merges) that reaches the day-cap margin, merging only as many *pairs* of preferred-size blocks into double-size ones as actually needed — e.g. 4 hours needing to drop from 4 blocks to 3 becomes `[2, 1, 1]` (one merge) rather than `[2, 2]` (every block merged). Doubling — not a flat "+1" step — is the only upgrade that exactly preserves total hours when merging two same-size blocks into one, so the base and upgrade sizes are both derived from `component_block_rule`'s actual configured `preferredBlockSize` for `Core`: an admin changing that value in Settings' Block Rules changes what this does (e.g. `preferredBlockSize=2` would merge pairs into 4h blocks instead of 1h into 2h) without any code change. A leftover remainder block from packing at the base size (when hours don't divide evenly) is never itself a merge candidate. Deliberately caps out at double the base size by design — per the explicit choice made when this was built, it never escalates further even when that's not enough: if `baseSize * 2` would already exceed the 4h structural maximum (base size 3 or 4), or even every full-size block merged still isn't bare-feasible, `decomposeHours` falls back straight to the untouched naive shape, never trying a uniform bigger shape the way every other designation does.
