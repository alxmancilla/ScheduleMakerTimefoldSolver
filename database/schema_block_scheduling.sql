...
-- ============================================================================
-- Per-constraint soft-weight overrides, read by DataLoader into a Timefold
-- ConstraintWeightOverrides at solve time. A constraint with no row here
-- keeps its hardcoded default from scheduler-common's SoftConstraintDefaults
-- (the single canonical list of known soft constraint names/defaults,
-- shared by engine and web) - this table only ever holds explicit
-- overrides, not a full copy of every known constraint's weight.
