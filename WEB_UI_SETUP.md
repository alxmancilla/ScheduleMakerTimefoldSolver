...
| `PUT /assignments/{id}/move` | `SCHEDULER`/`ADMIN` | Move/pin a block without touching anything else — sets only `{blockTimeslotId, pinned}`, re-validating server-side (same checks as `validate-move`) before saving. A move implies a pin: changing the slot with `pinned:false`, or setting a slot on an already-unpinned block, is rejected with 400, since the current-schedule view takes an unpinned row's position from the latest solver run and would ignore the new slot. Unpinning in place is allowed and clears the timeslot. The Schedule grid's move/pin editor's save path for a day/hour/pinned-only change; a room/teacher change instead goes through the general `PUT /assignments/{id}` above. |
